...
const SelectedWorkspace:React.FC<SelectedWorkspaceProps> = ({workspace,onClick}) => {
    ...
    const [workspaceLogo, setWorkspaceLogo] = useState('/wedoclogo.svg')
  ...
  return (
    <Link href={`/dashboard/${workspace.id}`} onClick={()=>{
        if(onClick) onClick(workspace)
    }}
    className='flex rounded-md hover:bg-muted transition-all flex-row p-2 gap-4 justify-center my-2 cursor-pointer'>
