	...
	return (
		<header className='p-4 flex justify-center items-center relative'>
			<Link href={'/'} className='w-full flex gap-2 items-center '>
				<Image src={Logo} alt='WeDoc logo' width={25} height={25} />
				<span className='font-semibold dark:text-white'>Wedoc</span>
			</Link>
				...
				<NavigationMenuList className='gap-6'>
					<NavigationMenuItem>
						<NavigationMenuTrigger onClick={() => setPath('#resources')}
							className={cn({
								'dark:text-white': path === '#resources', 'dark:text-white/40': path !== '#resources',
								'font-normal': true, 'text-xl': true
							})}>
							Resources
						</NavigationMenuTrigger>
						<NavigationMenuContent>
							<ul className='grid gap-3 p-6 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr]'>
								<li className='row-span-3'>
									<span className='flex h-full w-full select-none flex-col justify-end bg-gradient-to-b from-muted/50
									to-muted p-6 no-underline outline-none focus:shadow-md'>Welcome</span>
								</li>
					...
					<NavigationMenuItem>
						<NavigationMenuTrigger onClick={() => { setPath('#pricing') }}
							className={cn({
								'dark:text-white': path === '#pricing', 'dark:text-white/40': path !== '#pricing',
								'font-normal': true, 'text-xl': true
							})}>
							Pricing
						</NavigationMenuTrigger>
					...
					<NavigationMenuItem>
						...
						<NavigationMenuContent>
							<ul className='grid w-[400px] gap-3 p-4 md:w-[500px] md:grid-row-2 lg:w-[600px]'>
								{components.map((component, i) => {

									return <ListItem href={component.href} key={i} title={component.title}>
			...
			<aside className='flex -w-ful -gap-2 justify-end'>
				<Link href={'/login'}>
